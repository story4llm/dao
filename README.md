...
《周易》的核心价值不在于给市场贴上某个卦名，而在于它把世界理解为一个不断变化的过程。《系辞》将阴阳爻之间的相互替换视为变化发生的基础；八卦、六十四卦及其顺序，则构成了一种由状态、关系和转换组成的认知结构。现代研究也通常把《易经》理解为一种过程性、关系性和情境化的变化哲学。
...
道家把“道”理解为万物自身运行的过程，强调行动应适应事物的自然结构，而不是凭主观意志强行改变结果。
...
《孙子兵法》先比较条件、衡量力量，再决定是否进入行动。“先为不可胜，以待敌之可胜”“胜兵先胜而后求战”强调先建立不败条件，再等待优势出现；“虚实”强调识别力量真正薄弱和强盛之处。
...
Superforecasting相关研究强调拆分问题、参考基础概率、持续更新、聚合不同判断，并使用Brier Score等方式检验概率是否准确，而不是只统计方向对错。严格适当评分规则也被广泛用于概率预测评估，因为它们鼓励预测者诚实报告真实概率。
...
Hamilton提出的Markov状态转换模型，将时间序列参数视为受离散状态过程控制，为经济和金融中的状态切换建模提供了重要基础。
...
临界转变研究提出了方差上升、自相关增强和恢复速度变慢等潜在预警信号，但这些信号并非在所有系统中都可靠，也可能产生误报。因此它们更适合成为“脆弱性指标”，而不能单独成为转折预测器。
...
ReAct将推理与外部工具调用交替进行；Reflexion通过结果反馈和情景记忆修正后续行为；多Agent辩论则试图利用不同角色之间的质疑提高推理质量。现代Agent工程还强调工具边界、任务交接、结构化输出、护栏、追踪和评估。
...
CFTC的COT报告按交易者类别提供期货和期权持仓结构，其数据可作为中周期仓位特征，但因为它反映每周特定时点的持仓，不能直接作为实时信号。
...
CME提供黄金期货、合约规格、成交量、持仓及期权隐含波动相关数据，但实时数据通常涉及交易所授权与商业许可。
...
FRED可以提供包括10年期通胀保值国债实际收益率在内的官方利率时间序列。
...
World Gold Council的Goldhub包含金价、波动率、相关性、需求、央行和黄金市场数据，可用于中长期资金与需求分析。
...
美联储官网提供FOMC会议、声明、纪要和讲话日历，可作为政策事件的第一手来源。
